...
var loomSE = (function () {
	...
	var helper = {
		...
		newDOMobject: function (parent, type, id, cssClass, cssProperties) {
			...
			var newObject = document.createElement(type),
				newObjectId = _applicationId + '_' + id;
	...
	var gui = (function () {
		...
		btnFForward.element.clickEvent = function () {
			if (media.object.paused === false) {
				var time = media.getCurrentTime() + _behaviour.media.fastForwardSkip;
	...
	var readScript = (function () {
		...
		function setScene(scriptObject, scene) {
			...
			_currentScene = new Scene(scene, _behaviour.settings.language, scriptObject.scenes[scene]);

			//check if subtitles should be on
			if (_behaviour.settings.subtitles === true) {
				subtitles.on();
			} else {
				subtitles.off();
			}
			...
			for (var i = 0; i < array.length; i += 1) {
				...
				Event.prototype.schedule = function () {
					...
					var that = this,
						timeIn = that.in,
						timeOut = that.out,
						timeInLow = timeIn - _behaviour.media.timeEventResolution / 2,
						timeInHigh = timeIn + _behaviour.media.timeEventResolution / 2,
						timeOutLow = timeOut - _behaviour.media.timeEventResolution / 2,
						timeOutHigh = timeOut + _behaviour.media.timeEventResolution / 2;

					media.listen(function (time) {
						if (time >= timeInLow && time <= timeInHigh) {
							if (_behaviour.developer.verbose === 'full') {
		...
		function parse(url) {
			...
			helper.ajaxRequest(url, null, true, function (data) {
				if (data !== false) {
					...
					isActive = false;
					if (_behaviour.developer.verbose === 'subtitles') {
	...
	var media = (function () {
				...
				run: function (object) {
					...
					pollEvent = setInterval(function () {
						...
						if (oldTime !== newTime) {
							...
							if (_behaviour.developer.verbose === 'full' || _behaviour.developer.verbose === 'minimal') {
		...
		function create(container, media, callback) {
			...
			var Video = function () {
				...
				element.setAttribute('id', _applicationId + '_video');
				...
				if (_behaviour.developer.mute === true) {
					element.muted = true;
				}
			...
			if (media.type === 'audio') {
				...
				//environment.scaleVideo(media.video.width, media.video.height, _behaviour.media.scaleVideoTo);
				callback(object);
	...
	publicInterface.initialise = function (target, scriptFile, firstScene, resolution, callback) {
		...
		helper.ajaxRequest('js/behaviour.json', 'JSON', true, function (returnedData) {
			...
			helper.ajaxRequest(scriptFile, 'JSON', true, function (returnedData) {
				...
				readScript.setScene(_script, firstScene);
				gui.load();
